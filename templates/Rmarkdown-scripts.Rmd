...
```{r histogram, dev = "png", dpi = 300, fig.width=5, fig.height=4, fig.cap="Histogram plot", eval = F}
hist(rnorm(100))
```

<!-- ![example picture](./figures/histogram-1.png){width=90% height=80%} -->

```{r histgram example, echo=FALSE, fig.cap="Histogram plot", out.width = '100%', out.height="80%"}
...
```{tikz, a simple tikz-ex, fig.cap = "Tikz graph example", fig.ext = 'png', eval = T, echo = F}
...
```{tikz, CTMC graphs, fig.cap="A example graphs of CTMC model (fig.size cannot be changed via fig.width or fig.height)", fig.align="center", code = xfun::read_utf8("./graphics/CTMC.tex")}
```   
