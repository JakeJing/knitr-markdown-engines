...
```{r histogram-rng, dev = "png", dpi = 300, fig.width=5, fig.height=4, fig.cap="Histogram plot"}
hist(rnorm(100))
```

We can also co-refer a plot in Figure \@ref(fig:histogram-rng). Pls avoid using underscore (`_`) or white space in the co-reference labels. Likewise, you can also use `\@ref(tab/eq:)` to refer to the specific table or equation.


<!-- ![example picture](./figures/histogram-1.png){width=90% height=80%} -->

```{r histgram example, echo=FALSE, fig.cap="Histogram plot", out.width = '100%', out.height="80%", eval = F}
knitr::include_graphics("./figures/histogram-1.png")
```

Discussion
==========

```{tikz, simpletikz, fig.cap = "Tikz graph example", fig.ext = 'png', eval = T, echo = F}
...
```{tikz, simpletikz, fig.cap = "Tikz graph example (reuse by its labels; doesn't work for read_utf8)", echo = F}
```
